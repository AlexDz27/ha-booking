...
var currentRoomNumber = getSelectedOptionFromSelect(roomsNumberSelect).value;
var maxGuestsPermitted = capacityToMaxGuestsMap[currentRoomNumber];
roomsNumberSelect.addEventListener('change', function() {
  currentRoomNumber = getSelectedOptionFromSelect(roomsNumberSelect).value;
...
function correlateRoomsWithCapacity() {
  ...
  var options = getSelectOptions(capacitySelect);
  ...
  if (getSelectedOptionFromSelect(capacitySelect).disabled) {
    var possibleOptions = [];
